...
- **Hardware-created NaN sign is undefined** (architecture picks the payload/sign bits). Golden and parity comparisons therefore **canonicalize NaNs** — a NaN bit-pattern is compared as "is NaN", not bit-for-bit — while all **non-NaN values remain bit-exact**.
...
This refines the "exact `Mth` tables" fidelity language: the requirement is per-platform bit-exact match to native Paper, with NaN width-for-width canonicalization, not a single architecture-independent run-to-run table.
