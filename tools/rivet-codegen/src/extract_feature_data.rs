...
pub(crate) fn run_extractor(repo_root: &Path, bundler: &Path, output: &Path) -> Result<String> {
    ...
    let helper_dir_arg =
        extract::path_to_utf8(&helper_dir, "WorldgenFeatureDataExtractor output directory")?;
    let helper_file_arg =
        extract::path_to_utf8(&helper_file, "WorldgenFeatureDataExtractor source")?;
    extract::run_cmd(
        &javac,
...
"#,
    ...
    let helper_dir_arg = extract::path_to_utf8(
        &helper_dir,
        "WorldgenFeatureDataExtractor classpath directory",
    )?;
    ...
    let output_arg = extract::path_to_utf8(output, "feature-data output")?;
