...
pub fn run(
    ...
    // table + tag network content (data/biomes_tags.json), and the packet
    // tables (written into rivet-protocol by packets::run). `mod.rs` declares
    // the registry-owned ones and is written last so it can never drift from
    // the files it names.
    ...
    let header = format!(
        "// Generated by `tools/rivet-codegen generate` from data/block_states.json +\n\
         ...
         // data/biomes_tags.json + data/block_behaviors.json (MC {}).\n\
...
/// `synchronized.rs` (the configuration-sync element tables), and
