...
//! - `minecraft:block_entity_type` — stored chunk NBT type resolution and the
//!   `ClientboundLevelChunkPacketData.BlockEntityInfo` registry codec.
...
//! [`crate::generate`]). Deferred surfaces (mob_effect, particle_type,
//! sound_event, ...) are not on the current byte or loaded-chunk paths, so
//! emitting them now would be speculative.
//!
//! `minecraft:worldgen/feature_size_type` is the one datapack (worldgen/*)
//! registry emitted: not for any M1 wire surface, but for the #394 by-name
//! codec's built-in identity — the two vanilla feature-size types
//! (`two_layers_feature_size`, `three_layers_feature_size`) that
//! `FeatureSizeTypeId` resolves. Its small table is compiled unconditionally
//! (like `block_entity_types.rs`) so the identity surface needs no `blocks`
//! feature; the broad `registries.rs` also carries it under `blocks` like every
//! other target.
//!
...
pub fn run(input_flag: Option<&Path>, output_flag: Option<&Path>) -> Result<()> {
    ...
    let block_entity_types = surfaces
        .iter()
        .find(|surface| surface.key == "minecraft:block_entity_type")
        .expect("validated target surface");
    fs::write(
        output.join("block_entity_types.rs"),
        render_block_entity_types(block_entity_types, &provenance),
    )
    .context("write generated/block_entity_types.rs")?;
    let feature_size_types = surfaces
        .iter()
        .find(|surface| surface.key == "minecraft:worldgen/feature_size_type")
        .expect("validated target surface");
    fs::write(
        output.join("feature_size_types.rs"),
        render_feature_size_types(feature_size_types, &provenance),
    )
    .context("write generated/feature_size_types.rs")?;
...
/// Render the small, dependency-free block-entity identity table separately.
/// Unlike the broad static-builtin tables, this file is compiled without the
/// `blocks` feature because the chunk packet codec needs its element type.
fn render_block_entity_types(surface: &Surface, source: &SourceProvenance) -> String {
    let mut out = format!(
        "// Generated by `tools/rivet-codegen generate` from data/reports/registries.json\n\
         // (vanilla RegistryDumpReport; MC {}, protocol {}, world {}).\n\
         // Source jar sha256 {}; provenance linked to data/reports/manifest.json.\n\
...
/// Render the small, dependency-free feature-size identity table separately.
/// Like the block-entity table this file is compiled without the `blocks`
/// feature: the #394 by-name-codec surface needs the two built-in feature-size
/// type identities (`two_layers_feature_size`, `three_layers_feature_size`) to
/// populate `FEATURE_SIZE_TYPE`.
fn render_feature_size_types(surface: &Surface, source: &SourceProvenance) -> String {
    let mut out = format!(
        "// Generated by `tools/rivet-codegen generate` from data/reports/registries.json\n\
         // (vanilla RegistryDumpReport; MC {}, protocol {}, world {}).\n\
         // Source jar sha256 {}; provenance linked to data/reports/manifest.json.\n\
...
mod tests {
    ...
    /// A minimal report covering every targeted surface.
    fn minimal_report() -> Value {
        serde_json::json!({
            ...
            "minecraft:block_entity_type": {
                "entries": {
                    "minecraft:furnace": { "protocol_id": 0 },
                    "minecraft:chest": { "protocol_id": 1 }
                },
                "protocol_id": 10
            },
            "minecraft:data_component_type": {
            ...
            "minecraft:worldgen/feature_size_type": {
                "entries": {
                    "minecraft:three_layers_feature_size": { "protocol_id": 1 },
                    "minecraft:two_layers_feature_size": { "protocol_id": 0 }
                },
                "protocol_id": 50
            }
        })
    ...
    fn duplicate_names_cannot_appear_in_json() {
        ...
        let json = r#"{
            ...
            "minecraft:block_entity_type": {"entries": {}},
    ...
    fn rendering_is_deterministic() {
        ...
        assert!(first.contains("BLOCK_ENTITY_TYPE_BY_ID"));
        assert!(first.contains("WORLDGEN_FEATURE_SIZE_TYPE_BY_ID"));
        let block_entities = surface(&surfaces, "minecraft:block_entity_type");
        let identities = render_block_entity_types(block_entities, &source);
        assert!(identities.contains("minecraft:furnace\" => Some(0u16)"));
        assert!(identities.contains("provenance linked to data/reports/manifest.json"));
