...
# `dwell`, `kick`, `load-world`, `loaded-world`, and `generated-world`
# subcommands always boot exactly one rivet-server — their server selection is
# pinned to Rivet even without `--server` (issues #157, #86, #316, #374, and the
# generated-world seed-42 acceptance contract) — and `--server rivet|both`
...
needs_rivet=0
case "${1:-}" in
  dwell | kick | load-world | loaded-world | generated-world) needs_rivet=1 ;;
esac
prev=""
for a in "$@"; do
