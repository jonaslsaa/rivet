#!/usr/bin/env bash
# Compile and run the Paper-side `DataConverterProbe` against the pinned Paper
# 26.2 runtime, regenerating `fixtures/dataconverter/dataconverter-foundation.json`
# (issue #535). The probe is a single-file Java class that touches only the
# value-leaf `ca.spottedleaf.dataconverter` foundation classes — the
# `DataConverter` version-encoding arithmetic and ordering, the
# `DataType`/`DataHook`/`DataWalker` dispatch contract, and the `types`
# abstract-layer defaults exercised over the real NBT backing — so no server
# boot is needed.
#
# The runtime classpath is the materialized paper-26.2.jar FIRST (its bundled,
# patched LogUtils must win over the library jars) plus every library jar. The
# default materialized runtime is the M0 fixture-boot runtime
# `tools/rivet-oracle/work/run/{versions,libraries}`; `RIVET_PAPER_RUNTIME_JAR`
# and `RIVET_PAPER_LIBRARIES` override it.
#
# Usage:
#   scripts/run_dataconverter_probe.sh [out-dir] [paper-pin]
#   RIVET_PAPER_RUNTIME_JAR=/path/to/versions/26.2/paper-26.2.jar \
#   RIVET_PAPER_LIBRARIES=/path/to/libraries scripts/run_dataconverter_probe.sh
#
# The default output directory is `fixtures/dataconverter/` (in place). The
# probe always writes `dataconverter-foundation.json` inside the given
# directory, and the manifest is regenerated there too.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUT_DIR="${1:-$ROOT/fixtures/dataconverter}"
PAPER_PIN="${2:-26.2-DEV-main@0a99345}"

RUNTIME_JAR="${RIVET_PAPER_RUNTIME_JAR:-$ROOT/work/run/versions/26.2/paper-26.2.jar}"
if [ ! -f "$RUNTIME_JAR" ]; then
  echo "materialized server jar not found: $RUNTIME_JAR" >&2
  echo "boot the M0 fixture server once (tools/rivet-oracle/README.md) or set" >&2
  echo "RIVET_PAPER_RUNTIME_JAR to a versions/26.2/paper-26.2.jar" >&2
  exit 1
fi

LIBS_DIR="${RIVET_PAPER_LIBRARIES:-$ROOT/work/run/libraries}"
LIBS="$(find "$LIBS_DIR" -name '*.jar' 2>/dev/null | tr '\n' ':')"
if [ -z "$LIBS" ]; then
  echo "no library jars under $LIBS_DIR" >&2
  exit 1
fi

CLASSES="$ROOT/.cache/dataconverter-java"
mkdir -p "$CLASSES"
CP="$RUNTIME_JAR:$LIBS$CLASSES"

javac -cp "$CP" -d "$CLASSES" "$ROOT/src/java/DataConverterProbe.java"
mkdir -p "$OUT_DIR"
java -Xms256M -Xmx2G -cp "$CP" DataConverterProbe \
  --output "$OUT_DIR" --paper "$PAPER_PIN"
# The probe hardcodes the output basename `dataconverter-foundation.json`
# (DataConverterProbe writes `output/dataconverter-foundation.json`), so the
# first argument selects the output directory, not a file. Hash what the probe
# actually wrote, never an assumed file path.
FIXTURE_FILE="$OUT_DIR/dataconverter-foundation.json"
echo "wrote $FIXTURE_FILE"

# Refresh the fixture manifest so the regenerated goldens hash matches what the
# gate's `rivet-oracle verify` expects (the text/worldgen kinds regenerate their
# manifests in-process; the dataconverter kind is script-driven, so the script
# owns it).
. "$ROOT/scripts/write_fixture_manifest.sh"
NOTE="Ground-truth samples of the DataConverter foundation classes (issue #535): DataConverter.encodeVersions/getVersion/getStep/encodedToString + LOWEST_VERSION_COMPARATOR ordering, DataType.convertOrOriginal, DataHook/DataWalker contract, ObjectType.getType boundaries (incl. the NPE on null), and the MapType/ListType default-method dispatch (setGeneric, getList(type), getOrCreateList/Map, getBoolean coercion) exercised over the real NBT backing. Captured from the pinned Paper 26.2 runtime via DataConverterProbe. Deterministic across boots."
write_fixture_manifest "$OUT_DIR" "dataconverter-foundation" "$PAPER_PIN" "$NOTE" "$FIXTURE_FILE"
