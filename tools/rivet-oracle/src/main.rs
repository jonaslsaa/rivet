...
//!      `--composed-noise` / `--generated-expected`. The `spline/` and `seq/`
//!      value-leaf goldens are regenerated by `scripts/run_spline_probe.sh` /
//!      `scripts/run_seq_probe.sh` (script-driven, no boot; not `regenerate`
//!      modes), so bare `regenerate` never refreshes them — re-run those scripts
//!      after a Paper re-pin.
//!
...
fn verify_all_fixture_kinds() -> Result<(), Error> {
    ...
    generated_expected::verify_generated_expected_step(
        &crate_dir().join("fixtures/generated-expected"),
    )?;
    println!(
