...
//!      `biome-temperature/` value-leaf goldens). Each kind carries an
//!      independent manifest, so kinds can grow without a format migration.
...
//!      payloads (also twin-boot, issue #51), the worldgen semantic samples, the
//!      text component-JSON corpus (Paper reference-oracle capture, issue #98),
//!      the generated-expected seed-42 handoff (also twin-boot), and the
//!      seed-42 FEATURES checkpoint (also twin-boot, level-33 forced tickets →
//!      minecraft:full, the serialization ceiling of the forced path — the
//!      block data of which is the FEATURES-decoration output), and the
//!      seed-42 LIGHT-stage checkpoint (also twin-boot, same forced-ticket
//!      capture whose FULL serialization carries the Starlight-computed sky
//!      nibbles and emptiness map — the LIGHT-stage output). Sub-select
//!      with `--m0` / `--m2` / `--full` /
//!      `--samples` / `--text` / `--composed-noise` / `--surface-column` /
//!      `--generated-expected` / `--features` / `--light`. The `spline/`, `seq/`, and
//!      `biome-temperature/` value-leaf goldens are regenerated by
//!      `scripts/run_spline_probe.sh` / `scripts/run_seq_probe.sh` /
//!      `scripts/run_biome_temperature_probe.sh` (script-driven, no boot; not
//!      `regenerate` modes), so bare `regenerate` never refreshes them — re-run
//!      those scripts after a Paper re-pin.
//!
...
//!   cargo run -p rivet-oracle -- generated-expected <seed> [--to <out> | --tamper]
//!                                                      # seed-42 generated-world ground-truth handoff:
//!                                                      # verify the committed golden, capture a fresh
//!                                                      # seed-<seed> handoff to <out>, or tamper control
//!   cargo run -p rivet-oracle -- regenerate            # regenerate all fixture kinds
//!                                                      # (sub-select: --m0/--m2/--full/--samples/--text/--composed-noise/--generated-expected/--light)
...
/// and `<root>/light` dirs are excluded from the generic walk: each golden is
/// load-bearing in every run and carries its own tri-state contract (wholly
/// absent is `Error::Unverified` (exit 3), partial/corrupt hard-fails (exit
/// 1)) — asserted after the generic kinds pass. A root with no generic kinds
/// still reaches those classifications.
fn verify_all_fixture_kinds_from(root: &Path) -> Result<(), Error> {
    ...
    // The generated-expected ground-truth handoff (PR #563): beyond the manifest
    ...
    // The seed-42 FEATURES checkpoint (PR #175/#232): beyond the manifest
    // hashes, assert the level-33-forced `minecraft:full` chunks' status (whose
    // block data IS the FEATURES-decoration output — FEATURES is the last
    // block-mutating status), the decoration-sample contract (tree blocks at the
    // chunk center), and non-vacuity (every chunk distinct from its
    // surface-only pre-FEATURES state). Absent -> UNVERIFIED (exit 3), never a
    // silent green.
    verify_features_step(&features_dir)?;
    // The seed-42 LIGHT-stage checkpoint (PR #175/#184): beyond the manifest
    // hashes, assert the level-33-forced `minecraft:full` chunks' Starlight
    // sky-lighting truth — the published sky nibbles (the arrays the client
    // consumes) and the sky emptiness map — pinned to 0a99345 provenance and
    // non-vacuous (real sky data, not all-zeros). Absent -> UNVERIFIED (exit
    // 3), never a silent green.
    verify_light_step(&light_dir)?;
    Ok(())
}
...
/// The seed-42 LIGHT fixture tree's presence state. The tri-state contract is
/// central so the gate and the CLI cannot disagree about how a partial or absent
/// tree classifies.
#[derive(Debug, PartialEq, Eq)]
enum LightFixtureState {
    /// Neither manifest.json nor light.json exists: the checkpoint is wholly
    /// absent and maps to `Error::Unverified` (exit 3), never a silent green.
    Absent,
    /// Exactly one of the two exists (or one is unreadable/empty): a partial or
    /// corrupt tree is a hard failure (exit 1), NOT the exit-3 missing-prereq
...
/// Classify the committed seed-42 LIGHT fixture tree by presence.
fn classify_light_fixtures(dir: &Path) -> LightFixtureState {
    let manifest = dir.join("manifest.json").is_file();
    let golden = dir.join(light_stage::FIXTURE_BASENAME).is_file();
    match (manifest, golden) {
        (true, true) => LightFixtureState::Present,
        (false, false) => LightFixtureState::Absent,
        _ => LightFixtureState::Partial,
    }
}

/// Verify the committed seed-42 LIGHT checkpoint under the tri-state
/// contract: wholly absent -> `Error::Unverified` (exit 3), partial/corrupt ->
/// hard failure (exit 1), both present -> full verify. Shared by the gate and
/// the CLI so the two paths classify identically.
pub(crate) fn verify_light_step(dir: &Path) -> Result<(), Error> {
    match classify_light_fixtures(dir) {
        LightFixtureState::Absent => Err(Error::Unverified(format!(
            "light fixtures {} are ABSENT (need both manifest.json and {} — the seed-42 \
             LIGHT-stage checkpoint and its per-chunk sky-lighting gate cannot verify (git \
             checkout or regenerate via --light); refusing to pass green without them",
            dir.display(),
            light_stage::FIXTURE_BASENAME
        ))),
        LightFixtureState::Partial => Err(Error::Manifest(format!(
            "light fixtures {} are PARTIAL (only one of manifest.json / {} exists) — a \
             half-present tree is corrupt, not an absent prerequisite; restore or regenerate \
             both files before verifying",
            dir.display(),
            light_stage::FIXTURE_BASENAME
        ))),
        LightFixtureState::Present => {
            light_stage::verify_light(dir)?;
            println!(
                "PASS: light seed-42 LIGHT-stage checkpoint verified (pinned Paper 0a99345 \
                 provenance, status {}, published sky nibbles + emptiness map, non-vacuity)",
                light_stage::EXPECTED_STATUS
            );
            Ok(())
        }
    }
...
/// generated-expected handoff, the seed-42 features checkpoint, and the seed-42
/// light checkpoint all force at
...
/// every dimension — the proof that forced generation actually ran. A
...
fn run_regenerate(only: &[&str], to: Option<&Path>) -> Result<(), Error> {
    ...
    if to.is_some() && !to_targets_single_booting_kind(only) {
        ...
        return Err(Error::Gate(format!(
            "regenerate --to <dir> requires exactly one of --m0/--m2/--full — bare \
             ...
             --composed-noise/--surface-column/--generated-expected/--features/--light (which \
             regenerate their committed fixture trees and ignore --to), are refused; got {what}"
        )));
    }
    ...
    let generated_expected = regenerates_kind("--generated-expected", only);
    let features = regenerates_kind("--features", only);
    let light = regenerates_kind("--light", only);
    ...
    if generated_expected {
        println!(
            "==> regenerating generated-expected seed-42 handoff from pinned Paper (twin-boot)"
        );
        generated_expected::run_probe(&crate_dir().join("fixtures/generated-expected"))?;
    }
    if features {
        println!(
            "==> regenerating features seed-42 FEATURES checkpoint from pinned Paper (twin-boot)"
        );
        features::run_probe(&crate_dir().join("fixtures/features"))?;
    }
    if light {
        println!(
            "==> regenerating light seed-42 LIGHT-stage checkpoint from pinned Paper (twin-boot)"
        );
        light_stage::run_probe(&crate_dir().join("fixtures/light"))?;
    }
    Ok(())
}
...
fn print_usage() {
    ...
    println!("  cargo run -p rivet-oracle -- generated-expected <seed> [--to <out> | --tamper]");
    println!(
        "                                             seed-42 generated-world ground-truth handoff:"
    );
    println!(
        "                                             verify the committed golden (provenance +"
    );
    println!(
        "                                             manifest hash + per-chunk sample contract),"
    );
    println!(
        "                                             or --to <out> capture a fresh seed-<seed>"
    );
    println!(
        "                                             forced-grid handoff from the pinned Paper"
    );
    println!(
    ...
    println!("                                             minecraft:full chunks (status +");
    println!(
        "                                             decoration-sample contract + non-vacuity),"
    );
    println!(
        "                                             or --to <out> capture a fresh seed-<seed>"
    );
    println!(
        "                                             FEATURES checkpoint from the pinned Paper"
    );
    println!(
        "                                             runtime; --tamper is the negative control"
    );
    println!("  cargo run -p rivet-oracle -- light <seed> [--to <out> | --tamper]");
    println!("                                             seed-42 LIGHT-stage oracle checkpoint:");
    println!("                                             verify the committed level-33-forced");
    println!("                                             minecraft:full chunks (Starlight sky");
    println!(
        "                                             nibbles + emptiness map + non-vacuity),"
    );
    println!(
        "                                             or --to <out> capture a fresh seed-<seed>"
    );
    println!("                                             LIGHT checkpoint from the pinned Paper");
    println!(
        "                                             runtime; --tamper is the negative control"
    );
    ...
    println!(
        "                                              --generated-expected / --features / --light;"
    );
    println!(
...
fn run() -> Result<(), Error> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("generated-expected") => {
            // The seed-42 generated-world ground-truth handoff (PR #563).
        ...
        Some("features") => {
            ...
            // minecraft:full (the forced path's ceiling), whose block data IS
            // the FEATURES-decoration output, plus the exact verifier.
            ...
            let rest: Vec<&str> = args.iter().skip(1).map(String::as_str).collect();
            features::run_cli(&rest)
        }
        Some("light") => {
            // The seed-42 LIGHT-stage oracle checkpoint (PR #175/#184): the
            // level-33 forced-ticket capture whose chunks serialize at
            // minecraft:full carrying Starlight-computed light arrays — the
            // published sky nibbles (what the client consumes) and the sky
            // emptiness map — plus the exact verifier.
