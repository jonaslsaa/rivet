...
//!      and the generated-expected seed-42 handoff (also twin-boot). Sub-select
//!      with `--m0` / `--m2` / `--full` / `--samples` / `--text` /
//!      `--composed-noise` / `--generated-expected`. The `spline/`, `seq/`, and
    ...
    if to.is_some() && !to_targets_single_booting_kind(only) {
        ...
        return Err(Error::Gate(format!(
            "regenerate --to <dir> requires exactly one of --m0/--m2/--full — bare \
             ...
             --composed-noise/--surface-column/--generated-expected (which \
