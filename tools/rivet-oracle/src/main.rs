...
//!      block data of which is the FEATURES-decoration output). Sub-select
//!      with `--m0` / `--m2` / `--full` /
//!      `--samples` / `--text` / `--composed-noise` / `--surface-column` /
//!      `--generated-expected` / `--features`. The `spline/`, `seq/`, and
...
//!                                                      # (sub-select: --m0/--m2/--full/--samples/--text/--composed-noise/--generated-expected)
...
/// `<root>/composed-noise`, `<root>/generated-expected`, and
/// `<root>/features` dirs are excluded: each golden has a strict
    ...
    while let Some(dir) = walk.pop() {
        if let Ok(entries) = fs::read_dir(&dir) {
            for entry in entries.flatten() {
                ...
                if path == composed_noise || path == generated_expected || path == features {
                    continue;
                }
...
/// `<root>/composed-noise`, `<root>/generated-expected`, and `<root>/features`
/// dirs are excluded from the generic walk: each golden is load-bearing in
/// every run and carries its own tri-state contract (wholly absent is
/// `Error::Unverified` (exit 3), partial/corrupt hard-fails (exit 1)) —
/// asserted after the generic kinds pass. A root with no generic kinds still
/// reaches those classifications.
    ...
    if to.is_some() && !to_targets_single_booting_kind(only) {
        ...
        return Err(Error::Gate(format!(
            "regenerate --to <dir> requires exactly one of --m0/--m2/--full — bare \
             ...
             --composed-noise/--surface-column/--generated-expected/--features (which \
...
fn print_usage() {
    ...
    println!("                                              --generated-expected / --features;");
    println!(
