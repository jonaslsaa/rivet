...
//!      payloads, the text component-JSON corpus, and the `spline/` value-leaf
//!      goldens). Each kind carries an independent manifest, so kinds can grow
...
//!      `--text`. The spline value-leaf goldens are regenerated by
//!      `scripts/run_spline_probe.sh` (script-driven, no boot; not a
//!      `regenerate` mode).
//!
