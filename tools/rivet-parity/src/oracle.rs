...
impl Oracle {
    ...
    pub fn call(&mut self, op: &str, fields: &[(&str, &str)]) -> Result<Value, String> {
        let response = self.request(op, fields)?;
        if response.get("ok").and_then(Value::as_bool).unwrap_or(false) {
            Ok(response
                .get("result")
                .cloned()
                .ok_or("oracle success without result")?)
        } else {
            let error = response.get("error").cloned().unwrap_or(Value::Null);
            Err(format!("oracle {op} failed: {error}"))
        }
    }

    /// The `ping` provenance block (Paper spec/impl/commit/sha/java).
    pub fn provenance(&mut self) -> Result<Value, String> {
        self.call("ping", &[])
    }
}
