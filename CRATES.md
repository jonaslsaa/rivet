...
**Data:** `serde` + `serde_json` (configs, datapack JSON — never packets or chunk hot paths), `toml` (config), `slotmap` (entity/holder arenas), `rustc-hash` (FxHashMap default), `indexmap` (order-observable maps), `smallvec`, `bitflags`, `phf` (generated static tables), `uuid`, `bytes`. `pcre2` backs `PathAllowList`'s trusted-admin `regex:` configurations: unlike Rust's finite-automata engines it supports Java Pattern's authorization-relevant lookarounds and backreferences, and its UTF-without-UCP mode preserves Java's default ASCII `\d`/`\w`/`\s` classes. A compatibility validator rejects syntax with different Java/PCRE2 meanings rather than broadening an allow rule; `pcre2-sys` vendors PCRE2 when a supported platform has no system library.
