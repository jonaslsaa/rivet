...
- **`Mth` transcendentals (`COS_TAB`/`ASIN_TAB`) are native-platform parity (D14).** Paper 26.2 builds them at class-init time with `java.lang.Math.cos`/`Math.asin`, whose HotSpot intrinsics differ by up to 1 ULP between x86_64 and aarch64 — there is no one bit-exact table Paper produces everywhere (exactly 11 of the 257 `COS_TAB` entries differ).
  ...
  - **`Math.min`/`Math.max` return the first operand for same-sign zero pairs.** For mixed-sign zeroes, `max` returns `+0.0` and `min` returns `-0.0`; port with explicit sign bits, never `f64::max`/`f32::min`.
