...
- **`Mth` transcendentals (`COS_TAB`/`ASIN_TAB`) are native-platform parity (D14).** Paper 26.2 builds them at class-init time with `java.lang.Math.cos`/`Math.asin`, whose HotSpot intrinsics differ by up to 1 ULP between x86_64 and aarch64 — there is no one bit-exact table Paper produces everywhere (exactly 11 of the 257 `COS_TAB` entries differ). Rivet carries **independently generated, compile-time selected** per-arch tables and matches **native Paper per supported platform**, not a single cross-platform constant. x86_64 Linux w/ Temurin 25 is the primary dev/release-gate target; aarch64 stays supported and must have its own verified generated table. Canonicalize floating golden bits with `Float.floatToIntBits`/`Double.doubleToLongBits` semantics (canonical NaN, every non-NaN bit exact) — matching the oracle. **`Math.min`/`Math.max` return the first operand for same-sign zero pairs** (force opposite sign only for mixed-sign zeroes); port with explicit sign bits, never `f64::max`/`f32::min`. **Never** substitute `StrictMath` and **never hand-edit** a generated table — regenerate on each target via the codegen path, which is fail-closed (host-arch-only writes, provenance-checked aarch64 table, `compile_error!` on unsupported arches).
