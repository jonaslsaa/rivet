...
//!
//! The registry-dependent `ByteBufCodecs` methods (`registry`, `holderRegistry`,
//! `holder`, `holderSet`, `registryFriendlyLengthPrefixed`, `fromCodec*`) and
//! the authlib/JOML/Gson value types are blocked on later units and are not
//! present here (documented in the `byte_buf_codecs` module). `Packet.codec` is
//! [`codec`].
//!
