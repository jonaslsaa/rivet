...
//! this flag belongs to (a reference-identity handle, see
...
///
/// Not `Copy`: the universe is an `Arc` identity handle, so the flag is
/// `Clone`. `PartialEq`/`Hash` follow the universe's pointer identity (see
/// `feature_flag_universe`).
