...
impl Heightmap {
    ...
    /// value has no handle to, so the port emits the same warning and returns
    /// `false` (data ignored), letting the chunk caller route the re-prime
    /// through [`ChunkAccess::prime_heightmaps`] (issue #287). Returns `true`
    /// when the data was copied in place — the only caller in the crate,
    /// `ChunkAccess::set_heightmap`, re-primes on `false` exactly like Java's
    /// `setRawData` mismatch branch.
    pub fn set_raw_data(&mut self, data: &[i64]) -> bool {
        let len = self.data.get_raw().len();
        if len == data.len() {
            self.data.get_raw_mut().copy_from_slice(data);
            true
        } else {
            // Java: `LOGGER.warn("Ignoring heightmap data for chunk {}, size
            // does not match; expected: {}, got: {}", ...)`.
            eprintln!(
                "Ignoring heightmap data, size does not match; expected: {len}, got: {}",
                data.len()
            );
            false
        }
...
mod tests {
    ...
    fn set_raw_data_ignores_mismatched_length() {
        // Java warns and re-primes on a length mismatch; the pure value warns,
        // ignores the data, and reports `false` so the chunk caller can route
        // the re-prime through `ChunkAccess::prime_heightmaps`. A shorter array
        // must not clobber or panic.
        let mut hm = Heightmap::new(384);
        assert!(!hm.set_raw_data(&[1, 2, 3]));
