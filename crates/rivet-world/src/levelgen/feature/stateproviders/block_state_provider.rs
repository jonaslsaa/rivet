...
/// carrier — dispatch an erased provider to its optional state (the
/// `BlockStateProvider.getOptionalState` surface the tree family's
/// `TrunkPlacer.placeBelowTrunkBlock` consumes; `RuleBasedStateProvider`
/// overrides the default to return `None` when no rule matches and no fallback
/// is present). The dispatch mirrors `block_state_provider_get_state`.
