...
//!
//! `simple()` mirrors Paper's `ONLY_IN_AIR_PREDICATE`
//! (`matchesTag(BlockTags.AIR)`) with a `MatchingBlockTagPredicate` over the
//! `minecraft:air` block tag at `Vec3i.ZERO` — the ported tag-matching
//! predicate (its `test_state` reads the generated `minecraft:air` tag's
//! members). The tag constant itself is not a named `BlockTags::AIR`; it is
//! built inline as `TagKey::create` over the block registry key. The world-access
//! `test` shell still resolves through the `#399` seam (see
//! `StateTestingPredicate::test`), but the predicate value and its codec wire
//! shape are faithful.
...
use rivet_registry::tag_key::TagKey;
...
impl BlockColumnConfiguration {
    ...
    ///
    /// `ONLY_IN_AIR_PREDICATE` is `matchesTag(BlockTags.AIR)` — a
    /// `MatchingBlockTagPredicate` at `Vec3i.ZERO` over the `minecraft:air`
    /// block tag (see the module doc). Java's `BlockTags.AIR` constant is a
    /// `TagKey.create(Registries.BLOCK, new ResourceLocation("air"))`; the port
    /// builds the same value inline.
...
/// }
/// ```
/// i.e. a `MatchingBlockTagPredicate` at `Vec3i.ZERO` over the `minecraft:air`
/// block tag. `BlockTags.AIR` is `create("air")` over the block registry key.
fn only_in_air_predicate() -> Arc<dyn BlockPredicate> {
    Arc::new(MatchingBlockTagPredicate::new(
        Vec3i::ZERO,
        TagKey::create(
            &*rivet_registry::registries::BLOCK,
            rivet_registry::Identifier::parse("minecraft:air"),
        ),
    ))
}
...
mod tests {
    ...
    use crate::levelgen::blockpredicates::matching_block_tag_predicate::MatchingBlockTagPredicate;
    use crate::levelgen::blockpredicates::state_testing_predicate::StateTestingPredicate;
    ...
    fn codec_round_trip() {
        ...
        // only-in-air predicate and a two-layer stack.
        ...
        assert_eq!(
            ...
            json!({
                ...
                "direction": "up",
                // `matchesTag(Vec3i.ZERO, tag)`: the default zero offset is
                // omitted on encode, so the wire shape is just `tag` (element
                // before the `type` key — `KeyDispatchCodec` encodes value
                // first, then the type key).
                "allowed_placement": {
                    "tag": "minecraft:air",
    ...
    fn simple_builds_the_up_column_shape() {
        ...
        // `ONLY_IN_AIR_PREDICATE` is `matchesTag(BlockTags.AIR)` — the
        // matching-block-tag predicate over `minecraft:air` at the zero offset.
        assert_eq!(
        ...
        let as_tag = config
            ...
            .expect("only-in-air predicate");
        assert_eq!(as_tag.offset(), &Vec3i::ZERO);
        assert_eq!(as_tag.tag().location().to_string(), "minecraft:air");
        // The `test_state` behavior is fully ported: air is in the tag, stone
        // is not.
        let stone_state = BlockState::of(BlockId::from_name("minecraft:stone").unwrap());
        assert!(as_tag.test_state(&air_state()));
        assert!(!as_tag.test_state(&stone_state));
    }
