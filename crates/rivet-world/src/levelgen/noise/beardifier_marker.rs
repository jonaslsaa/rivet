...
//! all return `0.0` (the enum's exact bodies), and the `BeardifierOrMarker`
//! unit codec (`MapCodec.unit(INSTANCE)`).
...
/// (everything `0.0`) and its unit codec.
...
/// `DensityFunctions.BeardifierOrMarker.CODEC` — `MapCodec.unit(INSTANCE)`,
/// erased to `MapCodec<Arc<dyn DensityFunction>>`. The `beardifier` dispatch
/// entry resolves here.
pub fn beardifier_marker_map_codec<Ops: DynamicOps + 'static>()
-> Arc<dyn MapCodec<Arc<dyn DensityFunction>, Ops>> {
    map_codec::xmap(
        map_codec::unit_with::<BeardifierMarker, Ops>(Arc::new(|| BeardifierMarker)),
        Arc::new(|m: &BeardifierMarker| -> Arc<dyn DensityFunction> { Arc::new(*m) }),
        Arc::new(|f: &Arc<dyn DensityFunction>| -> BeardifierMarker {
            f.as_any()
                .downcast_ref::<BeardifierMarker>()
                .copied()
                .unwrap_or_else(|| {
                    panic!("beardifier codec applied to a density function of a different type")
                })
        }),
    )
}
