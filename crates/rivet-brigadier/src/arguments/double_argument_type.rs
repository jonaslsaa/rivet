...
impl ArgumentType<f64> for DoubleArgumentType {
    fn parse(&self, reader: &mut StringReader) -> Result<f64, CommandSyntaxException<'static>> {
        ...
        if result < self.minimum {
            ...
            return Err(CommandSyntaxException::built_in_exceptions()
                ...
                .create_with_context(
                    reader,
                    &rivet_util::java_float_format::java_double_to_string(result),
                    &rivet_util::java_float_format::java_double_to_string(self.minimum),
                ));
        }
        if result > self.maximum {
            ...
            return Err(CommandSyntaxException::built_in_exceptions()
                ...
                .create_with_context(
                    reader,
                    &rivet_util::java_float_format::java_double_to_string(result),
                    &rivet_util::java_float_format::java_double_to_string(self.maximum),
                ));
        }
        Ok(result)
    }
    ...
    fn to_string(&self) -> String {
        let fmt = rivet_util::java_float_format::java_double_to_string;
