...
//! use the chunk's pending block-entity NBT authority as the narrow feature
//! seam needed here.
//!
//! ## Biome access
//!
    ...
    S: Eq + std::hash::Hash + Send,
{
    ...
    /// `WorldGenRegion.ensureCanWrite(BlockPos)` — the write-radius gate
    /// shared by the dense and FEATURES block-state specializations.
...
impl WorldGenRegion<'_, StateId, ServerBiomeId, StructureKey> {
    ...
    pub fn set_block(
        ...
        } else if is_feature_block_entity(BlockState::new(old_state)) {
...
impl WorldGenLevel for WorldGenRegion<'static, StateId, ServerBiomeId, StructureKey> {
    ...
    fn set_block(&mut self, pos: &BlockPos, state: BlockState, flags: u32) -> bool {
        WorldGenRegion::<StateId, ServerBiomeId, StructureKey>::set_block(
            self,
            pos,
            state,
            flags as i32,
    ...
    fn get_block_state(&self, pos: &BlockPos) -> BlockState {
        WorldGenRegion::<StateId, ServerBiomeId, StructureKey>::get_block_state(self, pos)
    }
...
/// decoration. `WorldGenLevel` is `'static`-bound in the current seam, so this
/// implementation is available to owning regions; the executor's borrowed
/// region keeps the generic read/write surface until the world-level ownership
/// seam removes that bound.
impl WorldGenLevel for WorldGenRegion<'static, BlockState, WorldgenBiomeId, StructureKey> {
    /// `WorldGenLevel.getSeed()`.
    fn get_seed(&self) -> i64 {
        self.seed
    }
    ...
    /// `WorldGenLevel.ensureCanWrite(BlockPos)` — the write-radius gate.
    fn ensure_can_write(&self, pos: &BlockPos) -> bool {
        WorldGenRegion::<BlockState, WorldgenBiomeId, StructureKey>::ensure_can_write(self, pos)
    }
    ...
    /// `LevelReader.getBiome(BlockPos)` — `getBiomeManager().getBiome(pos)`.
    /// The current `BiomeManager` owns the injected uncached source; cached
    /// chunk interpolation remains deferred with the holder seam.
    fn get_biome(&self, pos: &BlockPos) -> Holder<BiomeId> {
        self.get_biome_manager().get_biome(pos)
    }
...
/// The FEATURES-pass block-state read over the reconstructed `BlockState`
/// value type. `ProtoChunk` returns `VOID_AIR` outside the build height;
/// section reads return the stored state or ordinary air for an empty section.
...
/// The FEATURES-pass block-state write, including the persisted-status
/// heightmap update that `ProtoChunk.setBlockState` performs after the section
/// mutation.
...
mod tests {
    ...
    /// Feature block writes install Paper's lazy pending entity tag, and
    /// replacing the block removes that tag rather than inferring an entity
    /// from the block-state id alone.
    #[test]
    fn feature_block_writes_install_and_remove_lazy_entities() {
        let mut region = feature_region();
        ...
        let chest_tag = region
            .get_chunk(0, 0)
            .get_block_entity_nbt(&pos)
            .expect("chest tag remains present");
        assert_eq!(
        ...
        assert_eq!(chest_tag.get_long("LootTableSeed"), Some(17));
    ...
    #[test]
    fn fresh_spawner_persistence_uses_paper_defaults_and_clears_potentials() {
        let mut region = feature_region();
        let pos = BlockPos::new(0, 64, 0);
        assert!(region.set_block(&pos, Blocks::SPAWNER.default_block_state(), UPDATE_ALL, 0));
    ...
    #[test]
    fn zero_total_potentials_persist_spawn_data_without_weighted_rng() {
        let mut region = feature_region();
        let pos = BlockPos::new(0, 64, 0);
        assert!(region.set_block(&pos, Blocks::SPAWNER.default_block_state(), UPDATE_ALL, 0));
        let tag = region
        ...
        let tag = region
            .get_chunk(0, 0)
            .get_block_entity_nbt(&pos)
            .expect("zero-total spawner tag remains present");
        assert_eq!(
            tag.get_compound("SpawnData")
                .and_then(|data| data.get_compound("entity"))
                .and_then(|entity| entity.get_string("id"))
    ...
    #[test]
    fn dummy_spawner_payload_is_replaced_with_fresh_defaults() {
        let mut region = feature_region();
        let pos = BlockPos::new(0, 64, 0);
        assert!(region.set_block(&pos, Blocks::SPAWNER.default_block_state(), UPDATE_ALL, 0));
        let tag = region
    ...
    #[test]
    fn malformed_equipment_defaults_without_dropping_selected_potential() {
        let mut region = feature_region();
        let pos = BlockPos::new(0, 64, 0);
        assert!(region.set_block(&pos, Blocks::SPAWNER.default_block_state(), UPDATE_ALL, 0));
        let tag = region
        ...
        let spawn_data = region
            .get_chunk(0, 0)
            .get_block_entity_nbt(&pos)
            .and_then(|tag| tag.get_compound("SpawnData"))
            .expect("selected spawn data remains");
        assert_eq!(
            spawn_data
                .get_compound("entity")
                .and_then(|entity| entity.get_string("id"))
        ...
        assert!(
            spawn_data
                .get_compound("equipment")
                .is_some_and(|equipment| equipment.get("slot_drop_chances").is_none())
        );
    }

    #[test]
    fn malformed_custom_spawn_rules_drop_only_invalid_entries() {
        let mut region = feature_region();
        let pos = BlockPos::new(0, 64, 0);
        assert!(region.set_block(&pos, Blocks::SPAWNER.default_block_state(), UPDATE_ALL, 0));
        let tag = region
