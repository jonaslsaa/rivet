...
//! provider's [`ChunkAccessor`] and the write-back hooks, keeping the engine
//! itself free of a concrete chunk type. The visibility callback
//! (`updateVisible`'s `lightAccess.onLightUpdate`) is not ported: the engine
//! batches the *visible* state per chunk (via `SwmrNibbleArray::update_visible`)
//! and the provider owns the chunk, so there is no external listener to notify.
//!
...
//!   (`VoxelShape` is not); the `conditionally_full_opaque` flag is carried as
//!   the `FLAG_HAS_SIDED_TRANSPARENT_BLOCKS` and the flag-only paths are kept.
//!   For the superflat air + stone content neither block sets the flag, so the
//!   byte-exact contract is unaffected (the tests use only air/stone).
...
use crate::server::level::level_chunk::BiomeId as ServerBiomeId;
use crate::server::level::level_chunk::StateId;
...
use rivet_world::lighting::swmr_nibble_array::{InitState, SwmrNibbleArray, ARRAY_SIZE};
...
/// iteration). Table built at compile time for `bitset in 0..64`.
fn old_check_directions(bitset: usize) -> &'static [AxisDirection] {
    const TABLE: [&[AxisDirection]; 64] = {
        let mut table = [&[][..]; 64];
        ...
        while bitset < 64 {
            ...
            let mut list = [AxisDirection::PositiveX; 6];
            let mut remaining = bitset;
            let mut index = 0usize;
            while remaining != 0 {
                let trailing = remaining.trailing_zeros() as usize;
                list[index] = AxisDirection::ALL[trailing];
                index += 1;
                remaining &= remaining - 1;
            }
            let slice: &[AxisDirection] = match len {
                0 => &[],
                1 => {
                    const A: [AxisDirection; 1] = [AxisDirection::PositiveX];
                    const B: [AxisDirection; 1] = [AxisDirection::NegativeX];
                    const C: [AxisDirection; 1] = [AxisDirection::PositiveZ];
                    const D: [AxisDirection; 1] = [AxisDirection::NegativeZ];
                    const E: [AxisDirection; 1] = [AxisDirection::PositiveY];
                    const F: [AxisDirection; 1] = [AxisDirection::NegativeY];
                    match bitset {
                        1 => &A,
                        2 => &B,
                        4 => &C,
                ...
                2 => match bitset {
                    3 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::PositiveX, AxisDirection::NegativeX];
                        &V
                    }
                    5 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::PositiveX, AxisDirection::PositiveZ];
                        &V
                    }
                    6 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::NegativeX, AxisDirection::PositiveZ];
                        &V
                    }
                    9 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::PositiveX, AxisDirection::NegativeZ];
                        &V
                    }
                    10 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::NegativeX, AxisDirection::NegativeZ];
                        &V
                    }
                    12 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::PositiveZ, AxisDirection::NegativeZ];
                        &V
                    }
                    17 => {
                        const V: [AxisDirection; 2] =
                    ...
                    18 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::NegativeX, AxisDirection::PositiveY];
                        &V
                    }
                    20 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::PositiveZ, AxisDirection::PositiveY];
                        &V
                    }
                    24 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::NegativeZ, AxisDirection::PositiveY];
                        &V
                    }
                    33 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::PositiveX, AxisDirection::NegativeY];
                        &V
                    }
                    34 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::NegativeX, AxisDirection::NegativeY];
                        &V
                    }
                    36 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::PositiveZ, AxisDirection::NegativeY];
                        &V
                    }
                    40 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::NegativeZ, AxisDirection::NegativeY];
                        &V
                    }
                    48 => {
                        const V: [AxisDirection; 2] =
                            [AxisDirection::PositiveY, AxisDirection::NegativeY];
                        &V
                    }
                    _ => &[],
                }
...
pub(crate) trait ChunkAccessor {
    ...
    fn get_chunk_for_lighting(&mut self, chunk_x: i32, chunk_z: i32)
        -> Option<&ChunkAccess<StateId, ServerBiomeId, StructureKey>>;
}

/// The per-chunk surface the engine reads while propagating: the chunk's sky
/// nibbles and sky emptiness map. It owns both (the engine snapshot-copies them
/// into the cache), so a cached surface is independent of the chunk's lifetime.
#[derive(Clone, Debug)]
pub(crate) struct ChunkLightSurface {
    /// `starlight$getSkyNibbles()` — the nibbles owned by the chunk, indexed
    /// from `minLightSection` through `maxLightSection`.
    nibbles: Vec<SwmrNibbleArray>,
    /// `starlight$getSkyEmptinessMap()` — per-world-section emptiness, or
    /// `None` when the chunk has not run the light stage (lazy init).
    emptiness_map: Option<Vec<bool>>,
}

impl ChunkLightSurface {
    fn new(nibbles: Vec<SwmrNibbleArray>, emptiness_map: Option<Vec<bool>>) -> Self {
        ChunkLightSurface {
            nibbles,
            emptiness_map,
        }
    ...
    fn nibbles_mut(&mut self) -> &mut Vec<SwmrNibbleArray> {
        &mut self.nibbles
    }

    fn emptiness_map(&self) -> Option<&Vec<bool>> {
        self.emptiness_map.as_ref()
    }
}
...
pub(crate) struct SkyStarLightEngine {
    ...
    section_cache: Vec<Option<LevelChunkSection<StateId, ServerBiomeId>>>,
    /// `nibbleCache`, indexed like `sectionCache`.
    nibble_cache: Vec<Option<SwmrNibbleArray>>,
    /// `chunkCache`, indexed `x + 5*z + chunkIndexOffset`. Owned surfaces (the
    /// engine snapshot-copies the nibbles + emptiness map), never a borrowed
    /// chunk.
    chunk_cache: Vec<Option<ChunkLightSurface>>,
...
impl SkyStarLightEngine {
    ...
    pub(crate) fn new(accessor: &dyn LevelHeightAccessor) -> Self {
        ...
        SkyStarLightEngine {
            ...
            chunk_cache: (0..25).map(|_| None).collect(),
    ...
    fn setup_encode_offset(&mut self, center_x: i32, center_y: i32, center_z: i32) {
        ...
        self.chunk_section_index_offset =
            self.chunk_index_offset + 25 * self.chunk_offset_y;
    }
    ...
    fn setup_caches(
        ...
        self.setup_encode_offset((center_x >> 4) * 16 + 7, center_y, (center_z >> 4) * 16 + 7);
        let radius = if try_to_load_chunks_for_2_radius { 2 } else { 1 };
        for dz in -radius..=radius {
            for dx in -radius..=radius {
                ...
                self.set_chunk_in_cache(cx, cz, chunk);
                self.set_emptiness_map_cache(cx, cz, self.get_emptiness_map(chunk));
                if !is_two_radius {
                    self.set_blocks_for_chunk_in_cache(cx, cz, chunk);
                    self.set_nibbles_for_chunk_in_cache(cx, cz, self.get_nibbles_on_chunk(chunk));
                }
            }
        }
    }
    ...
    fn get_chunk_in_cache(&self, chunk_x: i32, chunk_z: i32) -> Option<&ChunkLightSurface> {
        self.chunk_cache[(chunk_x + 5 * chunk_z + self.chunk_index_offset) as usize].as_ref()
    }

    fn set_chunk_in_cache(
        &mut self,
        chunk_x: i32,
        chunk_z: i32,
        ...
        self.chunk_cache[idx] = Some(ChunkLightSurface::new(
            chunk.sky_nibbles().to_vec(),
            self.get_emptiness_map(chunk),
        ));
    }
    ...
    fn get_chunk_section(
        ...
        chunk_y: i32,
        chunk_z: i32,
    ) -> Option<&LevelChunkSection<StateId, ServerBiomeId>> {
        let idx = (chunk_x + 5 * chunk_z + 25 * chunk_y + self.chunk_section_index_offset) as usize;
        self.section_cache[idx].as_ref()
    }

    fn set_chunk_section_in_cache(
        &mut self,
        chunk_x: i32,
        chunk_y: i32,
        chunk_z: i32,
        section: Option<LevelChunkSection<StateId, ServerBiomeId>>,
    ) {
        let idx = (chunk_x + 5 * chunk_z + 25 * chunk_y + self.chunk_section_index_offset) as usize;
    ...
    fn set_blocks_for_chunk_in_cache(
        ...
        for cy in self.min_light_section..=self.max_light_section {
            let section = if cy >= self.min_section && cy <= self.max_section {
                let index = (cy - self.min_section) as usize;
                chunk.get_sections().get(index).cloned()
            } else {
                None
            };
            self.set_chunk_section_in_cache(chunk_x, cy, chunk_z, section);
        }
    }
    ...
    /// `AIR` for a null (unloaded / out-of-bounds) section. `StateId(0)` is air.
        ...
        match self.section_cache[section_index].as_ref() {
            Some(section) => {
                if section.has_only_air() {
                    ...
                    section.states().get(local_index)
                }
            }
            None => StateId(0),
        }
    ...
    fn get_block_state_at(&self, world_x: i32, world_y: i32, world_z: i32) -> StateId {
        let index = ((world_x >> 4) + 5 * (world_z >> 4) + 25 * (world_y >> 4)
    ...
    /// `resizeIncreaseQueue` — Java's 1.5x growth (min 4).
    fn resize_increase_queue(&mut self) {
        let len = self.increase_queue.len();
        let new_len = len.max(4) + (len.max(4) >> 1);
    ...
    /// `getEmptinessMap(chunk)` — the chunk's stored sky emptiness map (lazy
    /// init when `None`). The engine reads it from the concrete chunk; the
    /// center chunk's map is forced into the cache by the caller.
    fn get_emptiness_map_from_surface(&self, chunk: &ChunkLightSurface) -> Option<&Vec<bool>> {
        chunk.emptiness_map()
    }
    ...
    fn init_nibble(
        ...
        if chunk_y < self.min_light_section
            ...
            || self.get_chunk_in_cache(chunk_x, chunk_z).is_none()
        {
            return;
        }
        let nibble = self.get_nibble_from_cache_mut(chunk_x, chunk_y, chunk_z);
        if nibble.is_none() {
        ...
        let nibble_clone = *self.get_nibble_from_cache(chunk_x, chunk_y, chunk_z).unwrap();
        self.init_nibble_impl(chunk_x, chunk_y, chunk_z, extrude, nibble_clone);
    }
    ...
    fn init_nibble_impl(
        ...
        chunk_z: i32,
        extrude: bool,
        _curr: SwmrNibbleArray,
    ) {
        let Some(curr) = self.get_nibble_from_cache_mut(chunk_x, chunk_y, chunk_z) else {
            return;
        };
        if !curr.is_null_nibble_updating() {
            // already initialised
            return;
        }
        let Some(emptiness_map) = self.get_emptiness_map(chunk_x, chunk_z) else {
            // cannot delay nibble init for lit chunks, as we need to init to
            // propagate into them — fall back to the section content.
            self.init_nibble_from_sections(chunk_x, chunk_y, chunk_z, extrude);
            return;
        };
        // Determine the lowest non-empty world section of this column.
        let mut lowest_y = self.min_light_section - 1;
        for curr_y in (self.min_section..=self.max_section).rev() {
            if emptiness_map[(curr_y - self.min_section) as usize] {
                // empty
                continue;
            }
            lowest_y = curr_y;
        ...
        if chunk_y > lowest_y {
            ...
            let nibble = self.get_nibble_from_cache_mut(chunk_x, chunk_y, chunk_z).unwrap();
            nibble.set_non_null();
            nibble.set_full();
            return;
        }
        if extrude {
            ...
            for curr_y in (chunk_y + 1)..=self.max_light_section {
                let above = self.get_nibble_from_cache_mut(chunk_x, curr_y, chunk_z);
                if let Some(above) = above {
                    if !above.is_null_nibble_updating() {
                        let curr = self.get_nibble_from_cache_mut(chunk_x, chunk_y, chunk_z).unwrap();
                        curr.set_non_null();
                        let other = above.clone();
    ...
    /// The `getEmptinessMap == null` fallback of `SkyStarLightEngine.initNibble`
    /// — the emptiness map is not stored on the chunk yet (the phase-A
    /// `ChunkAccess` has no sky map field), so the lowest non-empty section is
    /// resolved from the section content instead. This is the path the
    /// light-chunk run takes (the center chunk's map is freshly allocated).
    fn init_nibble_from_sections(&mut self, chunk_x: i32, chunk_y: i32, chunk_z: i32, extrude: bool) {
        let Some(curr) = self.get_nibble_from_cache_mut(chunk_x, chunk_y, chunk_z) else {
            return;
        };
        if !curr.is_null_nibble_updating() {
        ...
        let mut lowest_y = self.min_light_section - 1;
        for curr_y in (self.min_section..=self.max_section).rev() {
            let current = self.get_chunk_section(chunk_x, curr_y, chunk_z);
            match current {
                Some(section) if !section.has_only_air() => {
                    lowest_y = curr_y;
                    break;
                }
                _ => {}
    ...
    /// sections by dropping them from the cache (after publishing their visible
    /// state, so the removed nulls don't carry stale data).
    fn rewrite_nibble_cache_for_skylight(&mut self) {
        for index in 0..self.nibble_cache.len() {
            let nibble = self.nibble_cache[index].as_mut();
            if let Some(nibble) = nibble {
                if nibble.is_null_nibble_updating() {
                    self.nibble_cache[index] = None;
                    // Java's updateVisible is a no-op for a not-dirty null (the
                    // visible state is already null), so nothing to publish.
                }
            }
        }
    }
    ...
    /// their nibbles init'd so propagation can cross the empty section.
    fn check_null_section(
        ...
        'search: for dz in -1..=1 {
            for dx in -1..=1 {
                if let Some(nibble) = self.get_nibble_from_cache(dx + chunk_x, chunk_y, dz + chunk_z) {
                    if !nibble.is_null_nibble_updating() {
    ...
    fn get_light_level_extruded(&self, world_x: i32, mut world_y: i32, world_z: i32) -> i32 {
    ...
    fn try_propagate_skylight(
        ...
        self.check_null_section(world_x >> 4, start_y >> 4, world_z >> 4, extrude_initialised);
        ...
        while start_y >= (self.min_light_section << 4) {
            ...
            let from_shape_blocked = above.conditionally_full_opaque();
            ...
            if current.conditionally_full_opaque() {
                // the shape test would decide; the seam breaks (occludes).
                break;
            }
            ...
            above = current;
            if self.get_nibble_from_cache(world_x >> 4, start_y >> 4, world_z >> 4).is_none() {
    ...
    fn propagate_neighbour_levels(
        ...
        for curr_section_y in (from_section..=to_section).rev() {
            let Some(curr_nibble) = self.get_nibble_from_cache(chunk_x, curr_section_y, chunk_z) else {
                continue;
            };
            for direction in AxisDirection::ONLY_HORIZONTAL {
                let neighbour_off_x = direction.x();
                let neighbour_off_z = direction.z();
                let Some(neighbour_nibble) = self
                ...
                for curr_y in (curr_section_y << 4)..=((curr_section_y << 4) | 15) {
                    ...
                    for _ in 0..16 {
                        let level = neighbour_nibble.get_updating(
                            (curr_x & 15) | ((curr_z & 15) << 4) | ((curr_y & 15) << 8),
                        );
                        if level > 1 {
                            self.append_to_increase_queue(
        ...
        while queue_read_index < queue_length {
            ...
            if queue_value & FLAG_HAS_SIDED_TRANSPARENT_BLOCKS == 0 {
                for &propagate in check_directions {
                    ...
                    let section_index = ((off_x >> 4) + 5 * (off_z >> 4) + 25 * (off_y >> 4)
                        + section_offset) as usize;
                    let local_index =
                        ((off_x & 15) | ((off_z & 15) << 4) | ((off_y & 15) << 8)) as usize;
                    let current_nibble = self.nibble_cache[section_index].as_ref();
                    let Some(current_nibble) = current_nibble else { continue };
                    let current_level = current_nibble.get_updating_index(local_index);
                    ...
                    current_nibble.set_index(local_index, target_level);
                    if target_level > 1 {
        ...
        while queue_read_index < queue_length {
            ...
            if queue_value & FLAG_HAS_SIDED_TRANSPARENT_BLOCKS == 0 {
                for &propagate in check_directions {
                    ...
                    let current_nibble = self.nibble_cache[section_index].as_ref();
                    let Some(current_nibble) = current_nibble else { continue };
                    let light_level = current_nibble.get_updating_index(local_index);
                    if light_level == 0 {
                        continue;
                    }
                    let block_state = self.get_block_state(section_index, local_index);
                    ...
                    current_nibble.set_index(local_index, 0);
                    if target_level > 0 {
    ...
    ///
    /// The `chunk` parameter is used only for the de-init/init neighbour
    /// section lookups (through the cache); the actual write-back of the map is
    /// deferred to the caller via `set_emptiness_map_on_surface` when `ret`.
    fn handle_empty_section_changes(
        ...
        if needs_init {
            ...
            self.set_emptiness_map_cache(chunk_x, chunk_z, fresh.clone());
            chunk_emptiness_map = Some(fresh.clone());
            ret = Some(fresh);
        }
        let mut emptiness_map = chunk_emptiness_map.as_mut().expect("map set above");
        ...
        for section_index in (0..emptiness_changes.len()).rev() {
            ...
            if value_boxed.is_none() {
                ...
                value_boxed = Some(section.is_none() || section.unwrap().has_only_air());
            }
        ...
        for section_index in (0..emptiness_changes.len()).rev() {
            ...
            for dz in -1..=1 {
                for dx in -1..=1 {
                    ...
                    for dy in (1..=-1).rev() {
                        self.init_nibble(dx + chunk_x, dy + section_y, dz + chunk_z, extrude, false);
                    }
                }
            }
        }
        ...
        for dz in -1..=1 {
            for dx in -1..=1 {
                ...
                'neighbour_loaded_search: for dz2 in -1..=1 {
                    for dx2 in -1..=1 {
                        if self.get_emptiness_map(dx + dx2 + chunk_x, dz + dz2 + chunk_z).is_none() {
                    ...
                    'neighbour_search: for dy2 in -1..=1 {
                        for dz2 in -1..=1 {
                            for dx2 in -1..=1 {
                                ...
                                if let Some(emptiness_map) = self
                                    .get_emptiness_map(dx + dx2 + chunk_x, dz + dz2 + chunk_z)
                                {
                                    ...
                                    let section = self
                                        .get_chunk_section(dx + dx2 + chunk_x, y, dz + dz2 + chunk_z);
    ...
    pub(crate) fn light(
        ...
        let chunk_x = chunk.get_pos().x();
        let chunk_z = chunk.get_pos().z();
        self.setup_caches(provider, chunk_x * 16 + 7, 128, chunk_z * 16 + 7, true, true);

        let nibbles = get_filled_empty_light((self.max_light_section - self.min_light_section + 1) as usize);
        // force current chunk into cache
        self.set_chunk_in_cache(chunk_x, chunk_z, chunk);
        self.set_blocks_for_chunk_in_cache(chunk_x, chunk_z, chunk);
        self.set_nibbles_for_chunk_in_cache(chunk_x, chunk_z, &nibbles);
        self.set_emptiness_map_cache(chunk_x, chunk_z, self.get_emptiness_map(chunk));
        ...
        self.set_nibbles_on_surface(nibbles);
        self.update_visible();
        self.destroy_caches();
    }
    ...
    fn light_chunk_impl(
        ...
        self.null_propagation_check_cache.iter_mut().for_each(|b| *b = false);
                ...
                .map_or(true, |s| s.has_only_air())
        {
            ...
            for direction in AxisDirection::ONLY_HORIZONTAL {
                ...
                for curr_y in (highest_non_empty_section << 4)..=((highest_non_empty_section << 4) | 15) {
                    let (mut curr_x, mut curr_z) = (start_x, start_z);
                    for _ in 0..16 {
                        self.append_to_increase_queue(
                            ((curr_x + (curr_z << 6) + (curr_y << 12) + encode_offset) as u64
        ...
        if needs_edge_checks {
            ...
            self.propagate_neighbour_levels(chunk, self.min_light_section, highest_non_empty_section);
            self.perform_light_increase();
        }
    }
    ...
    fn update_visible(&mut self) {
        for nibble in self.nibble_cache.iter_mut() {
            if let Some(nibble) = nibble {
    ...
    fn destroy_caches(&mut self) {
        ...
        self.chunk_cache.iter_mut().for_each(|s| *s = None);
