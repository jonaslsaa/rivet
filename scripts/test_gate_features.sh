...
# regression suite). The sandbox has no real Paper tree, so stub it to pass —
# the suite's own behaviour is covered elsewhere; here it only needs to not
# abort the gate on its pass path.
