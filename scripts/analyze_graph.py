...
SPLIT_NOTES: dict[str, str] = {
    ...
    "mc.world.level.levelgen.noise": (
        ...
        "ChunkAccess); the #287 serialization surface (the 4 FULL-status "
        "heightmaps written by SerializableChunkData.write) stays with this "
        "owning unit | "
    ...
    "mc.world.level.chunk.access": (
        ...
        "blockEntities map (setBlockEntity/getBlockEntity) and "
        "setBlockEntityNbt's containsKey guard defer with the block-entity "
        "unit. BulkSectionAccess is a chunk.access file deferred in place "
        "(owned by this unit, not ported this slice); the set-block/fluid "
        "accessors defer with the chunk-storage epic (#216), the ticks "
        "surface with the world.ticks unit, and the Starlight emptiness maps "
        "with the lighting units (#184)"
    ),
    "mc.world.level.chunk.support": (
