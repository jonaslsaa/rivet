...
SPLIT_NOTES: dict[str, str] = {
    ...
    "mc.world.level.chunk.generator": (
        "#185: the generator stack "
        "(ChunkGenerator/ChunkGenerators/ChunkGeneratorStructureState) the "
        "pipeline-facing generator wave builds on | #306 feature-shell wave: the "
        "abstract ChunkGenerator seam is proactively ported out-of-unit (trait "
        "ChunkGenerator in rivet-world::chunk::chunk_generator: getMinY/getGenDepth "
        "required abstract getters; the five worldgen lifecycle steps "
        "create_biomes/apply_carvers/build_surface/spawn_original_mobs/fill_from_noise "
        "are default Paper-grounded panic seams (RivetTodo #185) because their "
        "WorldGenRegion/StructureManager/generic-ChunkAccess parameters are "
        "unnameable in a dyn-safe trait and no caller exists yet: not required, so "
        "test impls need no stubs; faithful getSpawnHeight=64 / getFirstFreeHeight "
        "/ getFirstOccupiedHeight defaults (the two height reads delegating to the "
        "getBaseHeight seam); the world-surface reads "
        "(getSeaLevel/getBaseHeight/getBaseColumn/addDebugScreenInfo/getBiomeSource) "
        "and createStructures/createReferences are explicit Paper-grounded panic "
        "seams (RivetTodo #185; the biome-membership read names #178); "
        "levelgen::feature/placement/WorldGenerationContext/CarvingContext consume "
        "it as &dyn ChunkGenerator); the .chunk.generator wave must not re-port it "
        "and must reconcile the trait seams with the noisegen value-shell bodies "
        "(getSeaLevel/getBaseHeight/getBaseColumn/addDebugScreenInfo/fill_from_noise, "
        "RivetTodo #185 integration blocker); do NOT implement the "
        "NoiseBasedChunkGenerator realization, server wiring, status executor, or "
        "WorldGenRegion here"
    ),
    "mc.world.level.storage": (
