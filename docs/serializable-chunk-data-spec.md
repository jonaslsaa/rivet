...
256 columns are packed into 36 longs (`getRawData`) in every dimension.
...
|---|---|---|
| `Status` (absent, wrong-typed, or empty string) | `parse` returns **null**; caller drops the chunk (ChunkLoadTask.java L350-352) | L145-147 |
...
5. **Read a launcher-created world:** copy a 26.2 world save (read-only, never
   ...
   Paper does (absent, wrong-typed, or empty `Status` → null; newer
   `DataVersion` honored).
...
9. Absent, wrong-typed, or empty `Status` → `parse` null → chunk dropped.
